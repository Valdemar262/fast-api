...
class SubmitTransition(StatusTransitionStrategy):
    def __init__(self, session: AsyncSession) -> None:
        self.session = session

    def can_transition(self, statement: Statement, actor: User) -> bool:
        return statement.status == StatementStatus.DRAFT and statement.user_id == actor.id

    def execute(self, statement: Statement, actor: User) -> None:
        apply_status(self.session, statement, StatementStatus.SUBMITTED)
